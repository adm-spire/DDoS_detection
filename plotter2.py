...
entropy_values = []
window_numbers = []  # Track window numbers
...
for start in range(0, len(df), WINDOW_SIZE):
    ...
    window_numbers.append(start // WINDOW_SIZE)  # Assign window index
...
# ================== PLOT 2: Entropy vs Window Number ==================
plt.figure(figsize=(12, 6))
plt.plot(window_numbers, entropy_values[:, 0], label="Entropy (Source IP)", marker='o', linestyle='-', color='blue')
plt.plot(window_numbers, entropy_values[:, 1], label="Entropy (Source Port)", marker='s', linestyle='-', color='cyan')

# Labels and legend
plt.xlabel("Window Number")
plt.ylabel("Entropy Value")
plt.title("Entropy vs. Window Number")
plt.legend()
plt.grid(True)
plt.show()
